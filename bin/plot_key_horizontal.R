...
for (i in 1:length(kmers)){
    k_pos = c(k_pos, (findInterval(kmers[i], bins) + 0.5) * 10000)
}
...
kpPlotRegions(kp, data=cyto_gr, avoid.overlapping = FALSE, col=cmap[1:N],
              data.panel=2)
...
kpPlotMarkers(kp, chr=rep('key', length(k_pos)), x=k_pos, y=1,
              labels=k_labels, r0=0, r1=-0.5, pos=4, srt=270,
